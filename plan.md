...
## 2026-05-01 通讯录与生活篇核验更新

### Objective

依据 2026 年 5 月前可检索的官方资料，更新 `docs/contacts/index.md` 与 `docs/life/index.md`，清理 2023-2024 年遗留的过期信息，重点修正地址、电话、校区交通、作息、网络、水电热水、国际校区商业与场馆服务说明。

### User Value

- 让 `/contacts` 与 `/life` 成为 2026 年仍可直接使用的页面，而不是迁移后的历史草稿。
- 降低新生、在校生和教职工获取电话、班车、门禁、网络、食堂和生活服务信息时的试错成本。
- 将高时效信息明确标注来源与更新时间，减少“看了 Wiki 但仍然踩坑”的情况。

### Constraints

- 必须优先采用学校官网、广州国际校区官网、师生服务中心、网络与信息化办公室、图书馆、校医院、后勤/综合事务等官方来源。
- 对没有找到官方依据的号码、时刻或营业信息，宁可删除或降级为“请以公告为准”，不能保留未经证实的旧内容。
- 保持现有 VitePress 页面风格，不引入新的前端依赖。
- 不回退用户已在首页或样式中做过的其它修改；本轮仅改与 `/contacts`、`/life` 直接相关的文件。

### Assumptions

- [ASSUMPTION] 2026 年 5 月时，校区官网公开可检索到的最新完整学生班车通知仍以 2025-08-26 和 2025-02-20 两份公告为主；如后续发现 2026 春季学期公开公告，则以更晚公告替换。
- [ASSUMPTION] 广州国际校区当前官方地址仍以“广州市番禺区兴业大道东777号”为准；若与历史民间写法“新行东路777号”冲突，则采用官方地址。
- [ASSUMPTION] 部分心理咨询页面存在旧域名跳转问题，可引用仍可访问的官方学院/书院页面或官方搜索结果摘要作为辅助佐证，但页面正文会尽量使用可打开的官方链接。

### Affected Files

- `docs/contacts/index.md`
- `docs/life/index.md`
- `data/campuses.json`
- `plan.md`
...
1. 逐项核验三校区地址、常用电话、师生服务中心、国际校区一站式服务、图书馆、医务室、心理咨询与一卡通挂失信息。
2. 替换 `/contacts` 中过期或无来源的号码与班车表，并为关键表格追加更新时间和来源链接。
3. 将 `/life` 的作息图片改为文本表格，重写网络、水电热水、门禁、食堂商铺、体育预约、地图与交通章节。
4. 对高时效内容增加“以官方最新公告为准”的提示，并尽量给出稳定入口而不是一次性截图。
5. 运行格式、链接、frontmatter 和构建检查；启动本地开发服务器检查 `/contacts` 与 `/life` 页面渲染。
...
- `npm run check`
- `npm run docs:dev`
- 本地访问 `/contacts` 与 `/life`，检查表格、外链、内部链接和 Markdown 渲染

### Risks

- 校区班车、商铺营业、食堂假期安排具有明显学期性和节假日波动，即使来源官方，也可能在短期内再次调整。
- 广州国际校区部分“生活服务”页面会在同一栏目下同时保留常态信息和假期特例，整理时容易把暑假安排误写成学期常态。
- 部分官方页面存在旧域名、旧路径或搜索摘要可见但页面直开失效的情况，若引用方式不谨慎会导致站内出现失效链接。

### Rollback Notes

- 本轮改动集中在两个内容页和 `data/campuses.json`，如需回退，可仅恢复这三个文件到修改前版本，不影响主题、首页或部署配置。
